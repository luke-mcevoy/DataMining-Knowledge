...
weight_ho<-c(0.5,0.9,0.9,-0.5,-0.85,-0.85)
Mw_ho<-matrix(weight_ho,nrow=3, ncol=2)
...
input_outputL<- c(out_hiddenL)
is.vector(input_outputL)

out_outL<-  sigmoid (  input_outputL%*% Mw_ho )

## X: Back propegation calculation
Actual<-matrix(c(.8,.2) , nrow = 1, ncol = 2,byrow = FALSE)
eta<- .1
e<-out_outL-Actual
